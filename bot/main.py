...
async def expense(message:Message):
 ...
 amount,description,category=data;await api("/bot/expenses",message,"POST",{"amount":str(amount),"currency":"TJS","category":category,"description":description});await message.answer(f"Записал: {amount} TJS · {category} · {description}")
